...
public class OSWindow extends OSResponder implements OSCoding {
    ...
    {
	    OSRect result= new OSRect(aRect);
		OSSize theSize= result.getSize();
			// ATTN-070630 [HD]: Very approximate calculatation... Will need to use the interfaceEngine
			//	to get correct result for OS/GUI.
		theSize.width-= 10;
		theSize.height-= 10;
		return result;
    }
    ...
    static OSRect minFrameWidthWithTitle_styleMask(String aTitle, int aStyle)
    {
			// ATTN-070630 [HD]: Very approximate calculatation... Will need to use the interfaceEngine
			//	to get correct result for OS/GUI.
		OSRect result= new OSRect(new OSPoint(0, 0), new OSSize(50, 20));
		return result;
    }
    ...
    public void setContentView(OSView aView)
    {
    ...
    {
		OSApplication.sharedApplication().setKeyWindow(this);
	is_key= true;
    }

    void becomeMainWindow()
    {
		OSApplication.sharedApplication().setMainWindow(this);
	is_main = true;
    }
	...
	OSWindow oldKey= theApp.keyWindow();
	if (oldKey != null) oldKey.resignKeyWindow();
	becomeKeyWindow();
    }
	...
	OSWindow oldMain= theApp.mainWindow();
	if (oldMain != null) oldMain.resignMainWindow();
	becomeMainWindow();
    }

    void orderBack(Object sender)
    {
		OSApplication.sharedApplication().setBackWindow(this);
	visible = true;
    }

    void orderFront(Object sender)
    {
		OSApplication.sharedApplication().setFrontWindow(this);
		visible = true;
    }
    ...
    {
		OSApplication.sharedApplication().takeOutWindow(this);
	visible = false;
    }
    ...
    {
	visible= true;
	if (first_responder != null) first_responder.becomeFirstResponder();
